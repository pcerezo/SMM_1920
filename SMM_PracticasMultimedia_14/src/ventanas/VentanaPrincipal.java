...
public class VentanaPrincipal extends javax.swing.JFrame {
    ...
    private boolean reproduciendo; //Booleano para saber cuando hacer pausa y cuándo seguir
    ...
    private void initComponents() {
        ...
        barraFormas.add(botonStop);

        listaReproduccion.setMaximumSize(new java.awt.Dimension(64, 23));
        barraFormas.add(listaReproduccion);
        ...
        barraFormas.add(botonGrabar);
        barraFormas.add(jSeparator8);
        ...
        abrirVideo.setText("Abrir vídeo...");
...
//                B = rgb & 0xFF;
    ...
    private void menuGuardarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_menuGuardarActionPerformed
        ...
        FileFilter filterImagenes = new FileNameExtensionFilter("JPEG file", "jpg", "jpeg");
    ...
    private String getExtension(String nombreArchivo) {
        char[] ext = new char[nombreArchivo.length()];
        char aux;
        int k = 0;
        String extension;
        
        for(int i = nombreArchivo.length()-1; i >= 0; i--){ //Empezamos desde el final hasta llegar al primer punto
            
            aux = nombreArchivo.charAt(i);
            if (aux == '.'){ 
                for (int j = i+1; j < nombreArchivo.length(); j++){ //Estamos al principio de la extensión, vamos hasta el final
                    ext[k] = nombreArchivo.charAt(j);
                    k++;
                }
            }
    ...
    private void menuAbrirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_menuAbrirActionPerformed
        JFileChooser dlg = new JFileChooser();
        FileFilter filterImagenes = new FileNameExtensionFilter("JPEG file", "jpg", "jpeg", "png");
        FileFilter filterSonidos = new FileNameExtensionFilter("Sonidos", "au", "wav");
        ...
        if (resp == JFileChooser.APPROVE_OPTION) {
            try {
                ...
                if (filterImagenes.accept(dlg.getSelectedFile())){
                    File f = dlg.getSelectedFile();
                    ...
                    //Obtengo los gráficos de la imagen para poder dibujar sobre ella
                ...
                else if (filterSonidos.accept(dlg.getSelectedFile())) {
...
//        }
    ...
    private void listaColoresActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_listaColoresActionPerformed
        ...
        Color colorActual = (Color) this.listaColores.getSelectedItem();
    ...
    private void botonPlayActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_botonPlayActionPerformed
        //Comprobamos primero si hay una ventana de vídeo seleccionada
        VentanaInternaVideo viv = (VentanaInternaVideo)this.escritorio.getSelectedFrame();
        
        if (viv != null) {
            viv.play();
        }
        else{
            if (!reproduciendo && !pausa){ //Se ha pulsado STOP o no se ha reproducido nada
                File f = (File) listaReproduccion.getSelectedItem();
                try{
                    if (f != null) {
                        this.player = new SMClipPlayer(f);
                        this.player.addLineListener(new ManejadorAudio());

                        if (this.player != null) {
                            System.out.println("Comienzo");
                            //reproduciendo = true;
                            this.player.play(); //En el manejador ponemos reproduciendo = true
                        }
                    }
                }
                catch(Exception e){
            ...
            else{ //Si se está escuchando algo o está pendiente de seguir
                if (pausa) { //Justo antes habíamos pulsado pausa, la reproducción está pendiente
                    System.out.println("Reanudo");
                    this.player.resume();
                    this.pausa = false;
                }
                else { //Se está escuchando
    ...
    private void botonStopActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_botonStopActionPerformed
        //Compruebo si está activa una ventana interna de vídeo
        VentanaInternaVideo viv = (VentanaInternaVideo) this.escritorio.getSelectedFrame();
        ...
        if (viv != null){
            ...
            if (this.recorder != null) {
                System.out.println("Voy a parar");
                this.recorder.stop();
                System.out.println("Grabación parada");
                this.recorder = null;
                System.out.println("Grabación en null");
            }
    ...
    private void botonCapturarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_botonCapturarActionPerformed
        VentanaInternaCamara vic = (VentanaInternaCamara) this.escritorio.getSelectedFrame();
        
        if (vic != null) {
            BufferedImage img = vic.getImage();
            if (img != null) { //Creo una ventana para mostrar la captura
                VentanaInternaImagen vii = new VentanaInternaImagen();
                vii.getLienzo2D().setFondo(img); 
                vii.setTitle("[CAPTURA]");
                this.escritorio.add(vii);
                vii.setVisible(true);
            }
        }
        else {
            System.out.println("No hay imagen que capturar");
        }
    ...
    private void abrirVideoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_abrirVideoActionPerformed
        ...
        if (resp == JFileChooser.APPROVE_OPTION) {
            try {
                ...
                //Creo la ventana de vídeo
                VentanaInternaVideo viv = VentanaInternaVideo.getInstance(f);
                viv.addMediaPlayerEventListener(new ManejadorVideo());
    ...
    public static void main(String args[]) {
        ...
        NativeLibrary.addSearchPath(RuntimeUtil.getLibVlcLibraryName(), "/usr/share/");
        System.out.println(LibVlc.INSTANCE.libvlc_get_version());
        //boolean ok = new NativeDiscovery().discover();
        //if (!ok) System.err.println(ok + "No se pudo acceder a las bibliotecas de VLC");
    ...
    private class ManejadorAudio implements LineListener{
        ...
        public void update(LineEvent event) {
            if (event.getType() == LineEvent.Type.START) { //Reproduce
                //botonPlay.setEnabled(false);
                //Ponemos el icono de pausa en el botonPlay
                botonPlay.setIcon(new javax.swing.ImageIcon(getClass().getResource("/iconos/pausa24x24.png"))); //Cambio el icono al de pausa
                botonStop.setEnabled(true);
                reproduciendo = true;
            }
            if (event.getType() == LineEvent.Type.STOP) { //Pausa o Stop
                //botonPlay.setEnabled(true);
                //Cambiamos el icono al de play en el botonPlay
    ...
    private class ManejadorVideo extends MediaPlayerEventAdapter {
